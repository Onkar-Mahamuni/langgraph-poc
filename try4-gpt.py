...
Identify:
- intent: one of ["analyze_trend", "forecast_future", "estimate_cost", "recommend_scaling", "server_info", "unknown"]
- start_time and end_time (YYYY-MM-DD) if mentioned
- server_role if mentioned

ONLY return JSON like:
{{
  "intent": "forecast_future",
  "start_time": "2024-04-01",
  "end_time": "2024-04-15",
  "server_role": "web server"
}}

If unsure about intent, set intent as "unknown".
...
def fetch_data_node(state):
    ...
    azure_vm_df = fetch_azure_vm_specs()
...
Decide "up", "down", or "same" scaling.
...
graph.add_conditional_edges("Intent", lambda s: s["intent"], {
    ...
    "estimate_cost": "FetchData",
    "recommend_scaling": "FetchData",
...
graph.add_conditional_edges("Analyze", lambda s: s["intent"] in ["recommend_scaling", "estimate_cost"], {
    True: "Decide",
    False: END,
})
